...
def wandb_init(wandbcfg:config_dict.ConfigDict):
    ...
    if wandbcfg.resume is None:
        logger.error('wandbcfg.resume cannot be None when resuming, try "must".')
...
def main(_):
    ...
    with open(Path(FLAGS.run_path,'config.yml'),'r') as f:
        cfg = yaml.load(f, Loader=yaml.UnsafeLoader)
        logger.info('Successfully loadded configs')
    ...
    state = restore_trainingstate(FLAGS.run_path,'state')
    ...
    #         if np.min(loss_val) < best_run.summary['loss_val']:
